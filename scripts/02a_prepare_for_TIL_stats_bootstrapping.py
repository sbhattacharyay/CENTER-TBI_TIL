...
# Define number of missing value imputations
NUM_IMPUTATIONS = 100
...
# Initialise directory for storing bootstrapping resamples within results directory
bs_dir = os.path.join(results_dir,'bootstrapping_results','resamples')
os.makedirs(bs_dir,exist_ok=True)

### II. Draw resamples for bootstrapping
## First draw imputation indices
# Define set of possible imputations
imputation_indices = [i+1 for i in range(NUM_IMPUTATIONS)]

# Make random draws of imputation indices
imputation_draws = resample(imputation_indices,replace=True,n_samples=NUM_RESAMP)
...
TIL_validation_bs_resamples = pd.DataFrame({'RESAMPLE_IDX':[i+1 for i in range(NUM_RESAMP)],'IMPUTATION_IDX':imputation_draws,'GUPIs':TIL_validation_bs_rs_GUPIs})
...
# Create Data Frame to store bootstrapping resamples 
TIL_ICPEH_bs_resamples = pd.DataFrame({'RESAMPLE_IDX':[i+1 for i in range(NUM_RESAMP)],'IMPUTATION_IDX':imputation_draws,'GUPIs':TIL_ICPEH_bs_rs_GUPIs})

# Store TIL validation resamples
TIL_ICPEH_bs_resamples.to_pickle(os.path.join(TIL_ICPEH_dir,'TIL_ICPEH_resamples.pkl'))

## TIL-ICP_HR validation population
# Create sub-directory for TIL population bootstrapping results
...
# Create Data Frame to store bootstrapping resamples 
TIL_ICPHR_bs_resamples = pd.DataFrame({'RESAMPLE_IDX':[i+1 for i in range(NUM_RESAMP)],'IMPUTATION_IDX':imputation_draws,'GUPIs':TIL_ICPHR_bs_rs_GUPIs})
